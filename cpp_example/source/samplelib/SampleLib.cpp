...
static char* stringize (TaggedData* p)
{
	...
	{
	case kTypeUndefined:
		strncpy(temp,  "undefined", sizeof(temp));
		break;
	case kTypeBool:
		strncpy(temp, (p->data.intval == 0) ? "false" : "true", sizeof(temp) );
		break;
	case kTypeInteger:
		break;
	case kTypeUInteger:
...
extern "C" SAMPLIB long createArray (TaggedData* argv, long argc, TaggedData* result)
{
	...
	for (i = 0; i < argc; i++)
	{
		if (i > 0)
			strncat(script, ",", size);
		strncat(script, stringArgs[i], size);
	}
	strncat(script, "]", size);
...
extern "C" SAMPLIB long paramAny (TaggedData* argv, long argc, TaggedData* result)
{
	...
	case kTypeString:
		...
		{
			strncpy(result->data.string, argv[0].data.string, strlen (argv[0].data.string) + 1);
		}
		else
		{
...
extern "C" SAMPLIB long built(TaggedData* argv, long argc, TaggedData* result)
{
	...
	if  ( strlen(__DATE__) + strlen(__TIME__) < (sizeof(sResult)-10) )  {
		...
		sResult[0] = 0 ;
		strncpy(sResult, __DATE__, sizeof(sResult)) ;
		strncat(sResult,  " ", sizeof(sResult)) ;
		strncat(sResult, __TIME__, sizeof(sResult)) ;
